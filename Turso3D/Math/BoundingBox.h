...
namespace Turso3D
{
	...
	public:
		...
		BoundingBox(const Vector3& min_, const Vector3& max_) noexcept :
			min(min_),
			max(max_)
		{
		}
		...
		BoundingBox(float min_, float max_) noexcept :
		...
		bool Equals(const BoundingBox& box) const
		{
		...
		{
			Vector3 closest(
				std::max(std::min(point.x, max.x), min.x),
				std::max(std::min(point.y, max.y), min.y),
				std::max(std::min(point.z, max.z), min.z)
			);
		...
		std::pair<float, float> Projected(const Vector3& axis) const
		{
			...
			float edgeProj = std::abs(axis.Abs().DotProduct(edge));
