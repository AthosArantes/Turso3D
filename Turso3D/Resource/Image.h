...
namespace Turso3D
{
	...
	public:
		...
		// Regardless of original format, the image is saved as png.
		// Compressed image data is not supported.
		// Return true on success.
		bool Save(Stream& dest) override;

		// Set new image pixel dimensions and format.
		// Setting a compressed format is not supported.
		...
		// Return number of components in a pixel.
		// Will return 0 for formats which are not 8 bits per pixel.
		int Components() const { return components[format]; }
		// Return byte size of a pixel.
		// Will return 0 for block compressed formats.
		...
		// Calculate the next mip image with halved width and height.
		// Supports uncompressed 8 bits per pixel images only.
		// Return true on success.
		bool GenerateMipImage(Image& dest) const;
		// Return the data for a mip level.
		// Images loaded from eg. PNG or JPG formats will only have one (index 0) level.
		ImageLevel Level(size_t index) const;
		// Decompress a mip level as 8-bit RGBA.
		// Supports compressed images only.
		// Return true on success.
	...
	private:
		...
		static uint8_t* DecodePixelData(Stream& source, int& width, int& height, int& depth, unsigned& components);
