...
namespace Turso3D
{
	...
	public:
		...
		// which uses the non-threadsafe NodeImpl allocator.
		void InitShadowViews();
		// Setup the camera and parameters for a shadow view.
		// Directional light shadow view should be supplied the scene bounds for focusing.
		...
		// Return actual shadow map rectangle.
		// May be smaller than the requested total shadow map size.
	...
	public:
		...
		// Set color.
		// Alpha component contains specular intensity.
		void SetColor(const Color& color);
		// Set range.
		void SetRange(float range);
		// Set spotlight field of view.
		void SetFov(float fov);
		// Set fade start distance, where 1 represents max draw distance.
		// Only has effect when max draw distance has been defined.
		...
		// Set directional light shadow view minimum size.
		// This is to prevent unwanted too sharp focusing e.g. when camera is facing the ground.
